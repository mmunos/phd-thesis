...
$.
Finally, define $\Runs(\cT,w)$ as the set of all runs of $\cT$ over $w$. %For a given \dsepsabbr $\cD$, we say that a subrun $\rho\spanc{i}{j}$ is represented in a node $v\in\cD$ if $\out(\rho\spanc{i}{j})\in\L_{D}(v)$.
