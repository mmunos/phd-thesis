...
This dissertation extends the state of the art in enumeration algorithms for queries built from formal languages and MSO. 
...
Each of these is explored in one of the main three chapters of the thesis, whose results we explain in more detail below.


Besides the results themselves, the conceptual contributions are two-fold. First, we propose a novel algorithmic data structure suited for enumeration called Enumerable Compact Sets (ECS). It is based on d-DNNF circuits and leverages their suitability for enumeration~\cite{AmarilliBJM17}. The specifications are rather straightforward. Structurally, it is a binary DAG whose nodes serve as indices of abstract sets of answers, and which are separated into output nodes, union nodes and product nodes. Output nodes are leaves in the structure, and they index a singleton set of an output symbol; union nodes connect to other two nodes and its set is defined as the union of the its two children; and product nodes connect to a left and right child, and its set is defined as the set obtained by concatenating any answer in its left child with any answer in its right child. On top of this, it is embedded with an interface which allows three constant-time for three kinds of operations: first, one can add a single symbol to the ECS, which creates an output node; second, one can add a node which represents the union of two existing nodes in the ECS; and third, one can add a node which represents the product of two existing nodes in the ECS. Lastly, the data structure describes an enumeration algorithm for each node type that requires no further preprocessing time, and it guarantees output-linear delay enumeration of its set. The only additional condition is that every operation has to be performed on two nodes that satisfy a {\em duplicate-free} condition---meaning that the respective sets of answers did not overlap.

There exists a naive way of implementing this interface, which is to simply create a node and connect it to the two existing nodes. However, this implementation forsakes the sought enumeration bounds. Consider the following sequence of operations: add eight symbols $a_1,\ldots,a_8$ to the ECS, and the singleton of each one becomes associated to the nodes $u_1,\ldots,u_8$, respectively; then build a binary tree out of them, by defining $u_9$ as the union of $u_1$ and $u_2$, then $u_{10}$ as the union of $u_3$ and $u_4$; and so on until $u_{15}$ is the root of the tree. If we are allowed no further preprocessing from this point, then enumerating the set associated to $u_{15}$ with output-linear delay (in this case, constant delay) is virtually impossible. This example illustrates why the node types and the interface of allowed operations need to be defined separately; and also the algorithmic challenge in implementing this while satisfying all the requirements. Indeed, the success that ECS has seen in improving the state of the art is partly due to this possibility being a blind spot in the literature. 
We attribute most of its success in the interface itself, which allows an intuitive connection between boolean queries and queries with second-order predicates.
...
The first chapter concerns enumeration for nested queries. Its main result is a streaming enumeration algorithm for MSO queries on nested documents (namely, the encoding of an unranked tree) which enumerates with output-linear delay. 
...
The algorithm is framed as a streaming algorithm which requires constant time per read symbol; so altogether, it requires linear-time preprocessing, matching the bounds of MSO queries on trees in the literature~\cite{Bagan06, AmarilliBMN19}. The main improvement in the best-known results is in the incremental evaluation aspect, since the resulting enumeration data structure admits constant time updates when remitted to appending one new symbol to the document. There was existing work that does incremental evaluation by structurally changed the tree~\cite{AmarilliBMN19}, yet this work does general updates in logtime. 

The main result of the chapter can be stated as follows.
\begin{thm}
...
The second chapter about enumeration for context-free grammars. We propose the formalism of {\em annotated grammars}, which is an annotator that allows expressing queries with complex inputs on a grammar. The task is to enumerate the set $\llbracket \cG \rrbracket(w)$ for any non-annotated input word $w$. We show an algorithm for queries specified with an unambiguous annotated grammars; namely, for every word-annotation pair, the grammar has at most derivation tree.

Then, we study a particular class of annotated grammar, which we call {\em rigid}. These are annotated grammars for which any input word may only have the same derivation tree {\em shape} for every valid annotation. This can be understood in terms of leftmost derivation sequences, as every annotation will result in the same sequence, modulo replacing nonterminals by other nonterminals.

Lastly, we explore pushdown annotators, which are pushdown automata on annotated words. We study the class of {\em profile-deterministic} pushdown annotators. Intuitively, this means that for every word, the shape of the stack is the same for every valid annotation, and moreover, it can be deduced deterministically by reading the word.
...
We also show how this result directly applies to the Context-Free Spanners and the Extraction Grammars formalisms proposed in a work Peterfreund~\cite{liatpaper}, and improves on its result that solved enumeration with quintic-time preprocessing. 
...
The third chapter deals with enumeration on compressed documents. Specifically, the model of word compression that is used are straight-line programs: this is a grammar-based compression scheme which encodes a word as a context-free grammar that has only one output. The query is modeled by an {\em annotated automaton}, a regular word automaton for annotated words.

This chapter also contains an additional conceptual contribution to solve the problem. We show a way to adapt the ECS data structure to include a certain operator {\em shift}. This is structurally represented by a new node type, the {\em shift} node, which has only one child and contains a constant $k$. The set it represents is precisely the set of answer in its child, except every value contained in every answer is incremented by $k$.

The main result in the chapter is thus,

\begin{thm}
	Given an unambiguous regular annotator $\cA$ and a word $w$ compressed by an SLP $S$, we can enumerate the set $\llbracket \cA\rrbracket(w)$ with preprocessing in $O(|\cA|^3 \times |S|)$ and output-linear delay.
\end{thm}

This result improves, and also can be seen as an adaptation of, one by Schmid and Schweikardt~\cite{SchmidS21}. They solve the same task---albeit defined for regular spanners---, but with logarithmic delay in the uncompressed document. 

We also describe how to adapt a framework for incremental evaluation in the context of complex document editing, which was also formalized by Schmid and Schweikardt~\cite{SchmidS22}.
